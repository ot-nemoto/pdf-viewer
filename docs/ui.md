...
- URL の値は `encodeURIComponent` でエンコードして指定する。未エンコードのままクエリ付き URL（`?token=abc&v=1` など）や `+` を含む URL を渡すと、`&` で値が切られ `+` が空白に変換されて別の URL になるため、黙って開かずにエラーメッセージで拒否する
...
- 承認前は取得元へ一切通信しない（サイズ確認の HEAD も送らない）。リンクを開いただけで IP・UA が取得元に渡ることを防ぐため（CORS はリクエストの送信自体を止めないため、取得に失敗するホストでもアクセスは記録される）
- 「開く」で読み込みを開始し、「キャンセル」および Esc で Empty 状態へ戻る

**読み込み中・失敗時**

- 読み込み中は進捗（% とダウンロード総量）を表示する。総バイト数が不明な間は「読み込み中…」のみ表示する
- 読み込みに失敗した場合は、原因が配信元にある可能性を示すメッセージと、当該 URL を新しいタブで開く導線を表示する（ダウンロード後にドロップする従来フローへ誘導する）。導線には遷移先のオリジンを必ず明示する（ユーザー指定の URL への遷移を、行き先を隠したまま提供しない）
...
- 「幅に合わせる」: ページ幅を表示領域の幅に自動で合わせる（縦に長いページは縦スクロール）
  - 縦スクロールバーの出入りでページ幅が振動しないよう、従来型スクロールバー環境ではバー厚み分を常に差し引いた幅に合わせる（縦に収まるページでは右端にバー厚み分の余白が残る）
- 「高さに合わせる」: ページ高さを表示領域の高さに自動で合わせる（横が広いページは横スクロール）
  - 横スクロールバーの出入りでページ高さが振動しないよう、従来型スクロールバー環境ではバー厚み分を常に差し引いた高さに合わせる（横に収まるページでは下端にバー厚み分の余白が残る）
- 2つのフィットは排他（各トグルで切替、同じボタン再押下で解除）
- ウィンドウ / コンテナのリサイズに追従する
- 手動ズーム（＋/−/リセット）操作時はフィットを解除し、手動倍率に切り替わる
- ファイルを開いた直後は「高さに合わせる」をデフォルトとする
